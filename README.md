...
1. **EDA** — first step: drop the (synthetic) PII columns, since real CRM
   ...
   against the effects built into the generator here, before any modeling
   decision is made.
...
- **Top features:** `lead_score_at_creation` (the real CRM rating field) is
  ...
  triaged their queue day to day. Country/industry coefficients point the way
  the generator built them to — `country=Australia` positive,
  `country=Finland`/`Sweden`/`India` negative, `industry=Healthcare`/
  `Finance` positive. **This is a sanity check, not a discovery:** the
  synthetic data plants exactly these effects
  (`src/lead_constants.py`, logit scale: India −0.9, Australia +0.6,
  Nordics −0.4, regulated industries +0.5, each applied at both the
  qualification and the win stage), so the check is that the pipeline
  can recover what was put in. Magnitudes don't line up one-to-one
  (Finland −1.05 vs. −0.4 planted), and some large positive coefficients
  (`Chile`, `Singapore`, `Colombia`, `France`) correspond to no planted
  effect — those countries only get a random N(0, 0.15) modifier — so at
  ~350 positives they show how noisy individual dummy coefficients are.
  XGBoost tells a different story:
