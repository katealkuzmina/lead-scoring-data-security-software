...
Every function here is pure given an `rng`
...
def sample_lead_type(rng: np.random.Generator) -> tuple:
    ...
    synthetic before/after snapshot for the leakage teaching point,
...
def compute_win_logit(row: dict, country_modifier: float) -> float:
    ...
    average, conditional on qualification)."""
    logit = -3.2
...
def assign_owner(
    ...
    """MSP/reseller always goes to a regional manager; so does any
...
def compute_deal_amount(
    ...
    recognized revenue never included, applied to every deal."""
...
def resolve_lead_type_at_close(rng: np.random.Generator, lead_type_current: str, is_won: bool) -> str:
    ...
    deal was registered under that end client's name instead.
...
def apply_field_completeness(rng: np.random.Generator, leads_df: pd.DataFrame) -> pd.DataFrame:
    ...
    engaged the lead. `lead_type_current` is a special case: it's
...
def generate_leads(n: int = 15000, seed: int = 42, max_draws: int = 200000) -> tuple:
    ...
    n_days = 548  # ~18 months, inside the modeled 2019-2022 period
